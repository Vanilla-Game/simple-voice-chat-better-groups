...
- Every pull request re-checks the client against every version in `voicechat.tested` (compile plus a production-like client launch through Loom's `ClientProductionRunTask`, using its built-in Xvfb support and force-loading every mixin target class; `required: true` with `defaultRequire: 1` turns any missing injection point into a non-zero exit). This guards our own changes as much as Simple Voice Chat updates.
