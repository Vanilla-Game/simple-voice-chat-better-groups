...
- **A leader with a golden crown.** The group's creator leads it: only the leader kicks members and approves requests. The crown passes to the longest-standing member when the leader leaves, or by hand with `/vcgroup transfer`.
...
- `/vcgroup invite <player>` — send a clickable invite
- `/vcgroup accept` — join by clicking the invite
- `/vcgroup kick <player>` — remove a member (leader only)
- `/vcgroup transfer <player>` — hand leadership to another member
- `/vcgroup request <group>` — ask to join a password-protected group
